...
if __name__ == "__main__":
    ...
    model_trained_weights = torch.load('./logs/new_exp/grad_37.pt', map_location=lambda loc, storage: loc)
    
    params_name_encoder = [key for key in model_trained_weights if key.startswith('encoder')]
    model.encoder.load_state_dict(OrderedDict((key[8:], model_trained_weights[key]) for key in params_name_encoder))
    
    params_name_decoder = [key for key in model_trained_weights if key.startswith('decoder')]
    model.decoder.load_state_dict(OrderedDict((key[8:], model_trained_weights[key]) for key in params_name_decoder))
    ...
    #print("trainable_param_names\n",trainable_param_names)
    #print("untrainable_param_names\n",untrainable_param_names)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        ckpt = model.state_dict()
        torch.save(ckpt, f=f"{log_dir}/grad_{epoch}_{diff_loss.item():.4f}.pt")
